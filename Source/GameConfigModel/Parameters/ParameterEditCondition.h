...
public:
	...
	virtual void ParameterChanged(const ULocalPlayer* InLocalPlayer, UGameParameter* OwnerParameter) const
	{
	}

	/* Return TRUE if owner parameter can be edited */
	virtual bool CanEdit(const ULocalPlayer* InLocalPlayer, UGameParameter* OwnerParameter)
	{
		return true;
	}
...
protected:
	...
	void RaiseConditionChanged() const
	{
		OnEditConditionChangedEvent.Broadcast();
	}
};
